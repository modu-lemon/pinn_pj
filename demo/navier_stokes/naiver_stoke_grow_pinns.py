...
u_train = torch.tensor(u[idx, :], dtype=torch.float32, requires_grad=True).to(device)
v_train = torch.tensor(v[idx, :], dtype=torch.float32, requires_grad=True).to(device)
...
for i in tqdm(range(800)):
    def closure():
        ...
        # Precondition per-equation residuals
        l_fu = (f_u.pow(2)).mean()
        l_fv = (f_v.pow(2)).mean()
        precond.update([l_fu.detach(), l_fv.detach()])
        loss_pde, _ = precond.apply([l_fu, l_fv])

        loss_data = (u - u_train).pow(2).mean() + (v - v_train).pow(2).mean()
...
# Evaluate one snapshot as in baseline
snap = np.array([100])
x_star = X_star[:, 0:1]
y_star = X_star[:, 1:2]
t_star_snap = TT[:, snap]
...
t_star_snap = torch.tensor(t_star_snap, dtype=torch.float32, requires_grad=True).to(device)

psi_and_p = model(x_star, y_star, t_star_snap)
psi = psi_and_p[:, 0:1]
p_pred = psi_and_p[:, 1:2]
...
print('Errors: u {:.4f}, v {:.4f}, p {:.4f}'.format(error_u, error_v, error_p))

plt.figure(figsize=(4, 3))
plt.imshow((p_pred).reshape(50, 100), extent=[-3, 8, -2, 2], aspect='auto')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Predicted p(x,t) - GROW-PINN')
plt.colorbar()
plt.tight_layout()
plt.savefig('./ns_grow_pinns_pred.png')