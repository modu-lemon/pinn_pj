...
model = PINNDiffusion(in_dim=2, hidden_dim=512, out_dim=1, num_layer=4, num_diffusion_steps=100).to(device)
...
for i in tqdm(range(1000)):
    def closure():
        ...
        t_diff = torch.randint(0, model.num_diffusion_steps, (x_res.shape[0],), device=device)
        ...
        # Diffusion loss
        loss_diff = model.p_losses(pred_res, t_diff)

        loss_track.append([loss_res.item(), loss_ic.item(), loss_bc.item(), loss_diff.item()])

        # Total loss with diffusion
        loss = loss_res + loss_ic + loss_bc + 0.1 * loss_diff
        optim.zero_grad()
        loss.backward()
        return loss
...
torch.save(model.state_dict(), './1dwave_pinns_diffusion.pt')

# Visualize PINNs with Diffusion
...
with open('1d_wave_pinns_diffusion_result.json', 'w') as f:
...
plt.colorbar()
plt.tight_layout()
plt.savefig('./1dwave_pinns_diffusion_pred.png')
plt.show()
...
plt.colorbar()
plt.tight_layout()
plt.savefig('./1dwave_pinns_diffusion_exact.png')
plt.show()

plt.figure(figsize=(4,3))
plt.imshow(np.abs(pred - u), extent=[0,1,1,0], aspect='auto')
plt.xlabel('x')
plt.ylabel('t')
plt.title('Absolute Error')
plt.colorbar()
plt.tight_layout()
plt.savefig('./1dwave_pinns_diffusion_error.png')
plt.show()
...
plt.figure(figsize=(4,3))
plt.imshow(samples, extent=[0,1,1,0], aspect='auto')
plt.xlabel('x')
plt.ylabel('t')
plt.title('Sampled u(x,t)')
plt.colorbar()
plt.tight_layout()
plt.savefig('./1dwave_pinns_diffusion_sample.png')
plt.show()